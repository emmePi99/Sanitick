...
export class MailerService {
  ...
  async sendActivationEmail(email: string, token: string): Promise<void> {
    ...
    await this.transporter.sendMail({
      ...
      html: `
        ...
        <p>Clicca sul link sottostante per attivare il tuo account e impostare la tua password:</p>
        <a href="${activationLink}" style="padding: 10px 20px; background-color: #007bff; color: white; text-decoration: none; border-radius: 5px; display: inline-block;">Attiva Account</a>
  ...
  // All'interno della tua classe MailerService...

async sendDoctorInvitation(email: string, firstName: string, lastName: string, token: string): Promise<void> {
  const frontendUrl = this.configService.get<string>('FRONTEND_URL');
  // Indirizziamo a una rotta specifica del frontend per l'impostazione della password
  const setupPasswordLink = `${frontendUrl}/imposta-password?token=${token}`;
